...
***The dataset has `r nrow(edibnb)` observations.***

2.  Run `View(edibnb)` in your Console to view the data in the data viewer. What does each row in the dataset represent?
***id, price, neighborhood, accomodates, bathrooms, bedrooms, beds, reviews scores ratings, number of
reviews, listing url.***
...
ggplot(data = edibnb, mapping = aes(x = price)) +
  ...
  facet_wrap(~neighbourhood, ncol = 5) +
   labs(title = "Airbnb Prices in the Neighbourhoods",
       x = "Airbnb Prices",
       y = "Frequency")
````

***I chose to use this layout because it makes the patterns between neighbourhoods clear. Wrapping them around makes more sense because then you can see all the graphs.***
...
top_neighborhoods <- edibnb %>%
  ...
  top_neighborhoods_vector <- top_neighborhoods %>%
    pull(neighbourhood)
...
````
````{r}
filtered_data <- edibnb %>%
  filter(neighbourhood==top_neighborhoods_vector)
ggplot(filtered_data, mapping = aes(x = price, y = neighbourhood, fill = neighbourhood)) +
  geom_density_ridges(scale = 1.2, alpha = 0.5, color="black") +
labs(
    title = "Distribution of Listing Prices in Top 5 Neighbourhoods",
    x = "Listing Price",
    y = "Neighbourhood"
  )
````
...
```
***Most Airbnb guests gave good ratings. There are too many neighbourhoods to distinguish one from another but most of their data is 90 or above. ***
